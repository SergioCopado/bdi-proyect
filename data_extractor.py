...
def buscar_libros(parametros_busqueda):
    ...
    try:
        ...
        for genero in parametros_busqueda:
            parametros = {'q': genero}
            # Realizar la solicitud para obtener el número total de resultados
            response = requests.get(url, params=parametros)
            data = response.json()

            if response.status_code == 200:
                resultados_totales += data['numFound']
                print(f"Se encontraron un total de {data['numFound']} resultados para el género {genero}.")

                # Iterar sobre todas las páginas de resultados
                for pagina in range(1, (data['numFound'] // 100) + 2):  # + 2 para asegurarse de obtener la última página
                    ...
                    if response.status_code == 200:
                        if data['docs']:
                            ...
                            print(f"No se encontraron libros en la página {pagina} para el género {genero}.")
                    else:
                        print(f"No se pudo obtener la página {pagina} de resultados para el género {genero}.")

        print(f"Se encontraron un total de {resultados_totales} resultados para todos los géneros proporcionados.")
...
# Ejemplo de uso
parametros_busqueda = ['science fiction', 'mystery', 'romance', 'action', 'fantasy', 'thriller', 'horror']
